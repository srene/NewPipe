...
package io.fluentic.ubicdn.net.bluetooth.discovery.ble.advertiser;
...
import io.fluentic.ubicdn.net.bluetooth.discovery.Advertiser;
...
public class BleAdvertiser implements Advertiser, BleAdvCallback.Listener
{
	...
	public void startAdvertise(long durationMs)
	{
		...
		if(advertiser == null)
		{
            G.Log(TAG,"ble peripheral failed to get BluetoothLeAdvertiser instance");
			queue.dispatch(new Runnable()
			{
				@Override
				public void run()
		...
		queue.dispatch(new Runnable()
		{
			...
			public void run()
			{
				G.Log(TAG,"ble advertise started");
	...
	{
        G.Log(TAG,"ble advertise failed: "+ BleAdvCallback.errorCodeToString(errorCode));
		stopAdvertise();
	}
	//endregion
