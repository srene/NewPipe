...
import io.fluentic.ubicdn.net.bluetooth.discovery.BtDiscoveryManager;
import io.fluentic.ubicdn.net.bluetooth.server.BtServer;
import io.fluentic.ubicdn.net.bluetooth.switcher.*;
import io.fluentic.ubicdn.util.G;
import io.fluentic.ubicdn.util.dispatch.DispatchQueue;

//import io.fluentic.ubicdn.net.bluetooth.pairing.BtPairer;

public class BtServiceDiscovery implements Discovery, BtServer.Listener, BtSwitcherDumb.Listener//, BtPairer.Listener
{
	private static final String TAG = "BtServiceDiscovery";
	// Uppercase.
	...
	private BtDiscoveryManager manager;
	private LinkListener listener;
	private BtSwitcher switcher;

	public BtServiceDiscovery(
			int appId,
			long nodeId,
	...
	//	this.pairer = new BtPairer(this, context);
		...
		G.Log(TAG,"bt adapter address " + BtUtils.getLocalAddress(context));

		int portsCountMax = uuids.size() / 2;
		this.server = new BtServer(appId, uuids, portsCountMax, adapter, pool, this, queue);

		manager = new BtDiscoveryManager(this, adapter, queue, context, uuids);
	} // BtServiceDiscovery()
	...
	public void startInternal()
	{
		...
		this.receiver = new BroadcastReceiver()
		{
			...
			{
				BtServiceDiscovery.this.onReceive(context, intent);
			}
		};
	...
	private void onReceive_ACTION_STATE_CHANGED(Intent intent)
	{
		...
		{
			G.Log(TAG,"bt bluetooth STATE_ON");

		//	switcher.setLegacy(!manager.isPeripheralSupported());
			if(!manager.isPeripheralSupported())
				G.Log(TAG,"BLE Peripheral mode is not supported on this device.");
		...
		{
			G.Log(TAG,"bt bluetooth STATE_OFF");
			manager.stop();
			stopListening();
	...
	private void checkScanModeAtStart()
	{
				...
				|| adapter.getScanMode() == BluetoothAdapter.SCAN_MODE_CONNECTABLE)
		{
			...
			G.Log("BTTransport","BT address="+BtUtils.getLocalAddress(context)+ " "+	adapter.getAddress() + " " + adapter +" " +BluetoothAdapter.getDefaultAdapter() + " " + BluetoothAdapter.getDefaultAdapter().getAddress());
			G.Log("BTTransport","BT bytesfromaddress="+BtUtils.getBytesFromAddress(BtUtils.getLocalAddress(context)));
			switcher.setMyAddress(BtUtils.getBytesFromAddress(BtUtils.getLocalAddress(context)));
			switcher.setLegacy(!manager.isPeripheralSupported());
			if(!manager.isPeripheralSupported())
				G.Log(TAG,"BLE Peripheral mode is not supported on this device.");
	...
	{
		G.Log(TAG, " requestBluetoothEnabled ");
		if(!running)
			return;
        G.Log(TAG, " requestBluetoothEnabled2 ");
        ...
        G.Log(TAG, " requestBluetoothEnabled3 ");

		final Discovery.AcceptCallback callback =
				new Discovery.AcceptCallback(){
					public void accept(final Activity activity)
					{
                        G.Log(TAG, " requestBluetoothEnabled4 ");
						...
						queue.dispatch(new Runnable()
						{
							...
							{
								// Main thread.
                                G.Log(TAG, " requestBluetoothEnabled5 ");
		...
		listenerQueue.dispatch(new Runnable()
		{
			...
			public void run()
			{
                G.Log(TAG, " requestBluetoothEnabled6 ");
                transportNeedsActivity(callback);
			}
		});
	} // requestDiscoverable()
	...
	public void onDeviceChannelsDiscovered(BluetoothDevice device, List<Integer> channels)
	{
		...
		G.Log(TAG,"Device discovered "+device.getName());
		if(isDeviceConnected(device))
			return;
	...
	public void onMustConnectAddress(byte[] address, List<Integer> channels)
	{
		...
		G.Log("BtServiceDiscovery","onMustConnectAddress");
		...
		for(int channel : channels)
		{
			if(channel < 0 || channel >= uuids.size())
				continue;
			G.Log("BtServiceDiscovery","onMustConnectAddress uuid "+uuids.get(channel));
	...
	{
		G.Log(TAG,"bt onMustSendFrame "+frame.getKind().toString()+" to "+BtUtils.getAddressStringFromBytes(address));
	...
	void linkDisconnected(final BtLink link, boolean wasConnected)
	{
		...
		if(wasConnected)
		{
			...
			listenerQueue.dispatch(new Runnable()
			{
				...
				public void run()
				{
					listener.btLinkDisconnected(link);
				}
			});
		}
	...
	void linkDidReceiveFrame(final BtLink link, byte [] frameData)
	{
		// Transport queue.
		if(!running)
			return;
		final byte [] fr = frameData;
		listenerQueue.dispatch(new Runnable()
		{
			@Override
			public void run()
			{
				listener.btLinkDidReceiveFrame(link, fr);
			}
		});
	}
	...
	void linkDidReceiveLinkFrame(BtLink link, Frames.Frame frame)
	{
		...
		{
			G.Log(TAG,"bt frame DISCONNECTED");
	...
	//region LinkListener
    //@Override
    public void transportNeedsActivity(final AcceptCallback callback)
    {
       ...
       /* AggLink aggregate = linksConnected.get(link.getNodeId());
        ...
        if(!linkExisted)
        {
            ...
            handler.post(new Runnable()
            {
                ...
                public void run()
                {
                    listener.btLinkConnected(AggTransport.this, finalLink);
                }
            });
        }*/
    } // btLinkConnected()
       ...
       /* AggLink aggregate = linksConnected.get(link.getNodeId());
        ...
        if(aggregate.isEmpty())
        {
            ...
            handler.post(new Runnable()
            {
                ...
                public void run()
                {
                    listener.btLinkDisconnected(AggTransport.this, finalLink);
                }
            });
        }*/
    } // btLinkDisconnected()
   ...
   // @Override
    public void transportLinkDidReceiveFrame(Link link, final byte[] frameData)
    {
        // Transport queue.
        /*AggLink aggregate = linksConnected.get(link.getNodeId());
        if(aggregate == null)
        ...
        handler.post(new Runnable()
        {
            ...
            public void run()
            {
                listener.btLinkDidReceiveFrame(AggTransport.this, finalLink, frameData);
            }
        });*/
