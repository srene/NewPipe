...
package io.fluentic.ubicdn.net.bluetooth.server;
...
{
	private final static String TAG="BtHacks";
	public static int getChannelOfBluetoothServerSocket(BluetoothServerSocket socket)
	{
		...
		for (Field field : f)
		{
			...
			{
				G.Log(TAG,"bt failed getChannelOfBluetoothServerSocket(): {}", ex);
				return -1;
			}
			...
			throws Throwable
	{
		...
		{
			G.Log(TAG,"bt failed custom listenUsingInsecureRfcommOn(ch={}): NoSuchMethodException " +channel);
			throw ex;
		}
		catch (IllegalAccessException ex)
		{
			G.Log(TAG,"bt failed custom listenUsingInsecureRfcommOn(ch={}): IllegalAccessException "+channel);
			throw ex;
		}
			...
			throws Throwable
	{
		...
		{
			G.Log(TAG,"bt failed custom createInsecureRfcommSocket(ch={}): NoSuchMethodException "+channel);
			throw ex;
		}
		catch (IllegalAccessException ex)
		{
			G.Log(TAG,"bt failed custom createInsecureRfcommSocket(ch={}): IllegalAccessException "+channel);
			throw ex;
		}
		catch (InvocationTargetException ex)
		{
