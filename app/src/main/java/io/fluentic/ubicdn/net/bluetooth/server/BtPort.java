...
public class BtPort
{
	private final static String TAG="BtPort";
	...
	public void close()
	{
		...
		{
			G.Log(TAG,"bt unlistening to uuid failed "+ ex);
		}
	}
	...
	private void listenUuids()
	{
		...
		{
			G.Log(TAG,"bt listening failed for uuid "+ uuid);
	...
	private void onAccept(BluetoothServerSocket oldServerSocket, BluetoothSocket clientSocket)
	{
		...
		{
			 G.Log(TAG,"bt server socket close failed. "+ ex);
		}
	...
	private static void accept(final BtPort port, final BluetoothServerSocket serverSocket)
	{
		...
		while(socket == null)
		{
			try
			{
				G.Log(TAG,"bt listening channel "+port.getChannel()+" uuid "+port.getUuid());
				socket = serverSocket.accept();
				G.Log(TAG,"bt accept channel "+port.getChannel()+" device "+ socket.getRemoteDevice() == null ? null : socket.getRemoteDevice().getName()
								+" "+socket.getRemoteDevice() == null ? null : socket.getRemoteDevice().getAddress());

			}
			catch (IOException ex)
			{
				G.Log(TAG,"bt accept() failed for uuid "+port.getUuid()+" "+ex);
